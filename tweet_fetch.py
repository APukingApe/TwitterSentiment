...
def fetch_tweets_by_region(query, geocode, max_tweets=1000, lang='en'):
    tweets_data = []
    tweet_count = 0
    max_id = None  

    logging.info(f"Starting tweet fetch for region: {geocode} and search {query}")
...
if __name__ == "__main__":
    ...
    geocode = "40.740209, -73.948064,50km"  # Replace with  target region in the format of: latitude,longitude,radius)
    max_tweets = 500  
    raw_tweets_by_region = "raw/tweets_data.csv"

    tweets_data = fetch_tweets_by_region(geocode, max_tweets)
    save_to_csv(tweets_data, raw_tweets_by_region)
